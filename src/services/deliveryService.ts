...
export const updateDeliveryInFirestore = async (deliveryId: string, deliveryData: Partial<Delivery>): Promise<{ success: boolean; error?: string }> => {
  try {
...
// Save delivery to Firestore (create or update)
export const saveDeliveryToFirestore = async (deliveryData: Partial<Delivery>): Promise<{ success: boolean; id?: string; error?: string }> => {
  try {
    if (deliveryData.id) {
      ...
      const updateResult = await updateDeliveryInFirestore(deliveryData.id, deliveryData);
      return { 
